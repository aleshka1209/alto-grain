<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>ТОО ALTO-GRAIN</title>
    <link rel="stylesheet" href="css/bootstrap-reboot.min.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/animate.css/4.1.1/animate.min.css"/>
    <link rel="stylesheet" href="css/style.css">
    <script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>
    <link rel="icon" href="favicon.png" type="image/x-icon">
</head>
<body>
    <header class="header">
        <div class="container">
             <div class="header__head">
                <div class="linkpage">
                    <a href="index.php" class="linkpage_l linkpage__active">RU</a>
                    <a href="#" class="linkpage_l">EN</a>
                </div>
                <div class="tel">
                    <a href="tel:7757990777" class="tel_number"><span class="light">+7 (7172)</span>  955 - 490</a>
                </div>
            </div>
            <div class="header__nav wow animate__fadeInDown" data-wow-duration="1s">
                <div class="header__logo">
                    <a href="index.php"><img src="/img/alto-logo.png" alt="logo"></a>
                </div>
                <ul class="header__menu" id="menu">
                    <li class="header__item"><a href="index.php">Главная</a></li>
                    <li class="header__item"><a href="about.php">О компании</a></li>
                    <li class="header__item"><a href="products.php">Продукция</a></li>
                    <li class="header__item"><a href="work.php"">Схема работы</a></li>
                    <li class="header__item"><a href="procurement.php">Закупки</a></li>
                    <li class="header__item"><a href="news.php">Новости</a></li>
                    <li class="header__item"><a href="contact.php">Контакты</a></li>
                </ul>
            </div>
            <div class="header__promo wow animate__bounceInUp" data-wow-duration="3s">
                <h1 class="header__h1">Закупка и экспорт</h1>
                <h2 class="header__h2">сельскохозяйственной продукции</h2>
            </div>
        </div>
    </header>